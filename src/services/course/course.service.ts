...
import { EnrolledCoursesRequest, EnrollCourseRequest, UpdateContentStateRequest, CourseBatchesRequest, BatchDetailsRequest, GetContentStateRequest } from "./bean"
...
export class CourseService {
    ...
    getContentState(requestObject: GetContentStateRequest){
        return new Promise((resolve, reject) => {
            this.factory.getCourseService().getContentState(JSON.stringify(requestObject), (success) => {
                resolve(success);
            }, (error) => {
                reject(error);
            });
